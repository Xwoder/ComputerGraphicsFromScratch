...
设计要点：
  1. 起始点 A = (0, 1)，斜率 k = 1/2，直线方程为 y = (1/2)x + 1。
  2. 用斜截式 y = kx + b 表示直线，斜率 k 与截距 b 直接给定。
  3. x 每次只递增 1 个网格单位（一个“点”），由 y = kx + b 得到浮点 y，
     再把 y 四舍五入到最近的网格行——即每次只“点亮”一个网格单元（最邻近栅格化）。
  4. 使用 Matplotlib 把被点亮的网格单元（实心方格）画在 100×100 网格上，
...
def rasterize_line(k, intercept, x_start, x_end):
    ...
    再四舍五入到最近的网格行，每个 x 只点亮一个网格单元 (x, round(y))。"""
    cells = []
    x = x_start
    while x <= x_end:
        y = k * x + intercept
...
def main():
    ...
    for k, b, color, x_end, name in LINES:
        ...
        # 栅格画法：每列只点亮一个最近的网格单元（仅保留落在 100×100 内的）
